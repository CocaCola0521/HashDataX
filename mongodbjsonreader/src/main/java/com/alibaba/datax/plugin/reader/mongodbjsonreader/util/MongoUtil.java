package com.alibaba.datax.plugin.reader.mongodbjsonreader.util;

import com.alibaba.datax.common.exception.DataXException;
import com.alibaba.datax.common.util.Configuration;
import com.alibaba.datax.plugin.reader.mongodbjsonreader.KeyConstant;
import com.alibaba.datax.plugin.reader.mongodbjsonreader.MongoDBJsonReaderErrorCode;
...
public class MongoUtil {
    ...
    public static MongoClient initMongoClient(Configuration conf) {
        ...
        if(addressList == null || addressList.size() <= 0) {
            throw DataXException.asDataXException(MongoDBJsonReaderErrorCode.ILLEGAL_VALUE,"不合法参数");
        }
        try {
            return new MongoClient(parseServerAddress(addressList));
        } catch (UnknownHostException e) {
            throw DataXException.asDataXException(MongoDBJsonReaderErrorCode.ILLEGAL_ADDRESS,"不合法的地址");
        } catch (NumberFormatException e) {
            throw DataXException.asDataXException(MongoDBJsonReaderErrorCode.ILLEGAL_VALUE,"不合法参数");
        } catch (Exception e) {
            throw DataXException.asDataXException(MongoDBJsonReaderErrorCode.UNEXCEPT_EXCEPTION,"未知异常");
        }
    }
